...
func (suite *FeeTestSuite) createCdps() {
	...
	// to get a ratio between 100 - 110% of liquidation ratio we can use 200xrp ($50) and 24 eurx (208% collateralization with liquidation ratio of 200%)
	// create CDP for the first address
	err := suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 200000000), c("eurx", 24000000))
	...
	err = suite.keeper.AddCdp(suite.ctx, addrs[1], c("xrp", 200000000), c("eurx", 10000000))
	suite.NoError(err) // check that no error was thrown
...
func (suite *FeeTestSuite) TestUpdateFees() {
	...
	// now check that we have the correct amount of fees overall (22 EURX for this scenario)
