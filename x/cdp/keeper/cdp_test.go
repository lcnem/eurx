...
func (suite *CdpTestSuite) TestAddCdp() {
	...
	err := suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 200000000), c("eurx", 26000000))
	suite.Require().True(errors.Is(err, types.ErrInvalidCollateralRatio))
	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 500000000), c("eurx", 26000000))
	suite.Error(err) // insufficient balance
	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 200000000), c("xeur", 10000000))
	...
	err = suite.keeper.AddCdp(suite.ctx, addrs[1], c("btc", 500000000000), c("eurx", 500000000001))
	...
	err = pk.SetCurrentPrices(ctx, "xrp:eur")
	suite.Error(err)
	ok := suite.keeper.UpdatePricefeedStatus(ctx, "xrp:eur")
	suite.False(ok)
	err = suite.keeper.AddCdp(ctx, addrs[0], c("xrp", 100000000), c("eurx", 10000000))
	suite.Require().True(errors.Is(err, types.ErrPricefeedDown))

	err = pk.SetCurrentPrices(suite.ctx, "xrp:eur")
	ok = suite.keeper.UpdatePricefeedStatus(suite.ctx, "xrp:eur")
	suite.True(ok)
	suite.NoError(err)
	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 100000000), c("eurx", 10000000))
	suite.NoError(err)
	...
	suite.Equal(cs(c("eurx", 10000000), c("xrp", 100000000), c("btc", 500000000)), acc.GetCoins())

	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("btc", 500000000), c("eurx", 26667000000))
	suite.Require().True(errors.Is(err, types.ErrInvalidCollateralRatio))

	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("btc", 500000000), c("eurx", 100000000))
	suite.NoError(err)
	id = suite.keeper.GetNextCdpID(suite.ctx)
	suite.Equal(uint64(3), id)
	tp = suite.keeper.GetTotalPrincipal(suite.ctx, "btc", "eurx")
	suite.Equal(i(100000000), tp)
	macc = sk.GetModuleAccount(suite.ctx, types.ModuleName)
	suite.Equal(cs(c("debt", 110000000), c("xrp", 100000000), c("btc", 500000000)), macc.GetCoins())
	acc = ak.GetAccount(suite.ctx, addrs[0])
	suite.Equal(cs(c("eurx", 110000000), c("xrp", 100000000)), acc.GetCoins())

	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("lol", 100), c("eurx", 10))
	suite.Require().True(errors.Is(err, types.ErrCollateralNotSupported))
	err = suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 100), c("eurx", 10))
...
func (suite *CdpTestSuite) TestGetSetCdp() {
	...
	cdp := types.NewCDP(types.DefaultCdpStartingID, addrs[0], c("xrp", 1), c("eurx", 1), tmtime.Canonical(time.Now()))
...
func (suite *CdpTestSuite) TestCalculateCollateralToDebtRatio() {
	_, addrs := app.GeneratePrivKeyAddressPairs(1)
	cdp := types.NewCDP(types.DefaultCdpStartingID, addrs[0], c("xrp", 3), c("eurx", 1), tmtime.Canonical(time.Now()))
	...
	cdp = types.NewCDP(types.DefaultCdpStartingID, addrs[0], c("xrp", 1), c("eurx", 2), tmtime.Canonical(time.Now()))
	cr = suite.keeper.CalculateCollateralToDebtRatio(suite.ctx, cdp.Collateral, cdp.Principal)
...
func (suite *CdpTestSuite) TestValidatePrincipal() {
	d := sdk.NewCoin("eurx", sdk.NewInt(10000000))
	err := suite.keeper.ValidatePrincipalAdd(suite.ctx, d)
	suite.NoError(err)
	d = sdk.NewCoin("xeur", sdk.NewInt(1))
	err = suite.keeper.ValidatePrincipalAdd(suite.ctx, d)
	suite.Require().True(errors.Is(err, types.ErrDebtNotSupported))
	d = sdk.NewCoin("eurx", sdk.NewInt(1000000000001))
	err = suite.keeper.ValidateDebtLimit(suite.ctx, "xrp", d)
	suite.Require().True(errors.Is(err, types.ErrExceedsDebtLimit))
	d = sdk.NewCoin("eurx", sdk.NewInt(100000000))
	err = suite.keeper.ValidateDebtLimit(suite.ctx, "xrp", d)
	suite.NoError(err)
}
...
func (suite *CdpTestSuite) TestCalculateCollateralizationRatio() {
	...
	c.AccumulatedFees = sdk.NewCoin("eurx", i(10000000))
	cr, err = suite.keeper.CalculateCollateralizationRatio(suite.ctx, c.Collateral, c.Principal, c.AccumulatedFees, "spot")
	suite.NoError(err)
